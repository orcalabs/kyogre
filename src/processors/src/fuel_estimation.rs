...
use kyogre_core::{
    ...
    NewFuelDayEstimate, PositionType, UpdateFuelMeasurementRange, Vessel, VesselEngine,
};
...
async fn vessel_task(
    ...
    while let Ok(vessel) = receiver.recv().await {
        ...
        process_fuel_measurment_ranges(&vessel, adapter.as_ref()).await;
    }
}

async fn process_fuel_measurment_ranges(vessel: &VesselToProcess, adapter: &dyn FuelEstimation) {
    let id = vessel.fiskeridir.id;
    if let Err(e) = process_fuel_measurment_ranges_impl(vessel, adapter).await {
        error!("failed to process fuel_measurement ranges for vessel_id: '{id}' err: {e:?}");
    }
}

async fn process_fuel_measurment_ranges_impl(
    vessel: &VesselToProcess,
    adapter: &dyn FuelEstimation,
) -> Result<()> {
    let ranges = adapter
        .unprocessed_fuel_measurement_ranges(vessel.vessel.fiskeridir.id)
        .await?;

    let mut output = Vec::with_capacity(ranges.len());

    for mut r in ranges {
        let pre_positions = adapter
            .ais_vms_positions_with_haul(
                vessel.fiskeridir.id,
                vessel.mmsi(),
                vessel.fiskeridir_call_sign(),
                &DateRange::new(r.fuel_range.start(), r.pre_estimate_ts)?,
            )
            .await?;

        let post_positions = adapter
            .ais_vms_positions_with_haul(
                vessel.fiskeridir.id,
                vessel.mmsi(),
                vessel.fiskeridir_call_sign(),
                &DateRange::new(r.post_estimate_ts, r.fuel_range.end())?,
            )
            .await?;

        let pre_fuel = estimate_fuel_for_positions(
            pre_positions,
            &vessel.engines,
            vessel.fiskeridir.service_speed,
            vessel.fiskeridir.degree_of_electrification,
        ...
        let post_fuel = estimate_fuel_for_positions(
            post_positions,
            &vessel.engines,
            vessel.fiskeridir.service_speed,
            vessel.fiskeridir.degree_of_electrification,
        ...
        // TODO: remove
        r.fuel_range.set_start_bound(kyogre_core::Bound::Exclusive);
        r.fuel_range.set_end_bound(kyogre_core::Bound::Exclusive);

        output.push(UpdateFuelMeasurementRange {
            fuel_range: r.fuel_range,
    ...
    adapter.add_fuel_measurement_updates(&output).await?;

...
impl FuelComputation for FuelImpls {
    fn fuel_liter(
        &mut self,
        first: &FuelItem,
        second: &FuelItem,
        ...
        match self {
            FuelImpls::Holtrop(holtrop) => holtrop.fuel_liter(first, second, vessel, time_ms),
            FuelImpls::Normal(normal_fuel) => {
                normal_fuel.fuel_liter(first, second, vessel, time_ms)
            }
        }
    }
    ...
    fn mode(&self) -> FuelMode {
        match self {
            FuelImpls::Holtrop(holtrop) => holtrop.mode(),
            FuelImpls::Normal(normal_fuel) => normal_fuel.mode(),
        }
    }
}
...
impl VesselFuelInfo {
    ...
    pub fn chose_fuel_impl(&self) -> Option<FuelImpls> {
        match self.mode {
            FuelMode::Normal => Some(FuelImpls::Normal(NormalFuel)),
            FuelMode::Holtrop => match (self.draught, self.length, self.breadth, self.main_sfc) {
                (Some(draught), Some(length), Some(breadth), Some(main_sfc)) => Some(
                    FuelImpls::Holtrop(Holtrop::new(draught, length, breadth, main_sfc)),
                ),
                _ => {
                    warn!(
                        "lacking vessel information for fuel estimation, vessel_id: {}, draught: {:?}, length: {:?}, breadth: {:?}, main_sfc: {:?}",
                        self.vessel_id, self.draught, self.length, self.breadth, self.main_sfc
                    );
                    None
                }
            },
    ...
    pub fn from_core(vessel: &Vessel, max_cargo_weight: Option<f64>, mode: FuelMode) -> Self {
        VesselFuelInfo {
...
pub async fn max_cargo_weight(
    ...
    match mode {
        FuelMode::Normal => Ok(Some(
            adapter
                .vessel_max_cargo_weight(vessel.fiskeridir.id)
                .await?,
        )),
        FuelMode::Holtrop => Ok(None),
    }
}

use crate::{FuelMode, Result, VesselFuelInfo, estimate_fuel_for_positions};
use chrono::Utc;
use kyogre_core::{
...
impl LiveFuel {
    pub fn new(adapter: Arc<dyn LiveFuelInbound>, mode: FuelMode) -> Self {
        Self { adapter, mode }
    }
    ...
    async fn compute_fuel_for_vessel(&self, vessel: &LiveFuelVessel) -> Result<Vec<NewLiveFuel>> {
        ...
        let vessel = VesselFuelInfo::from_live(vessel, self.mode);
        if let Some(mut fuel_impl) = vessel.chose_fuel_impl() {
            Ok(hour_split
                .into_iter()

...
WHERE
    ...
    AND current_trip_refresh_boundary IS NOT NULL
        ...
        for t in to_refresh {
            ...
            // We want to use the earlist timestamp for trip refresh boundaries and the latest
            // for current trip. t.timestamp will always be the earliest timestamp of the two as
            // its the refresh_boundary which always prioritizes the earlisest value.

...
impl ErsPorSet {
    ...
    fn add_species_group(&mut self, ers_por: &fiskeridir_rs::ErsPor) -> Result<(), PostgresError> {
        if let Some(code) = ers_por.catch.species.species_group_code {
            let species_group = ers_por.catch.species.species_group.clone().ok_or_else(|| {
                report!(PostgresError::DataConversion)
    ...
    fn add_species_main_group(
        ...
        if let Some(code) = ers_por.catch.species.species_main_group_code {
            let species_main_group = ers_por
                .catch
                .species
                .species_main_group
                .clone()

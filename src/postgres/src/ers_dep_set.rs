...
pub struct PreparedErsDepSet<'a> {
    ...
    pub catches: Vec<NewErsDepCatch<'a>>,
    pub ers_dep: Vec<NewErsDep<'a>>,
}

impl<'a> ErsDepSet<'a> {
    pub(crate) fn prepare(self) -> PreparedErsDepSet<'a> {
    ...
    pub(crate) fn new(ers_dep: impl Iterator<Item = &'a fiskeridir_rs::ErsDep>) -> Result<Self> {
        let (min, max) = ers_dep.size_hint();

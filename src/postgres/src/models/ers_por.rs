...
pub struct NewErsPor<'a> {
    ...
    pub landing_facility: Option<&'a str>,
...
impl<'a> From<&'a fiskeridir_rs::ErsPor> for NewErsPor<'a> {
    fn from(v: &'a fiskeridir_rs::ErsPor) -> Self {
        Self {
            ...
            landing_facility: v.landing_facility.as_deref(),
...
impl<'a> NewErsPorCatch<'a> {
    pub fn from_ers_por(ers_por: &'a fiskeridir_rs::ErsPor) -> Option<Self> {
        let c = &ers_por.catch;
        let s = &c.species;
            ...
            || s.species_main_group_code.is_some()
        {
            Some(Self {
                ...
                ers_quantum_type_id: ers_por.catch.quantum_type_code.as_deref(),
                living_weight: s.living_weight.map(|v| v as i32),
                species_fao_id: ers_por.catch.species.species_fao_code.as_deref(),
                species_fiskeridir_id: ers_por.catch.species.species_fdir_code,

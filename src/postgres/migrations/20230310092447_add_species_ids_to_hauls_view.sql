...
CREATE INDEX ON hauls_view (TSTZRANGE (start_timestamp, stop_timestamp));

CREATE INDEX ON hauls_view (gear_group_id);

CREATE INDEX ON hauls_view (species_group_ids);

CREATE INDEX ON hauls_view (vessel_length);

CREATE INDEX ON catch_locations USING GIST ("polygon");

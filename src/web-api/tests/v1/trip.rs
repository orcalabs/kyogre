...
async fn test_trip_of_haul_does_not_return_trip_of_other_vessels() {
    test(|helper| async move {
        ...
        let start = start + Duration::hours(1);
        let end = end - Duration::hours(1);
        let mut dca = ErsDca::test_default(1, Some(fiskeridir_vessel_id2.0 as u64));
        dca.vessel_info.call_sign_ers = "Other CallSign".try_into().unwrap();
        ...
        let haul = helper.db.generate_haul_from_ers_dca(dca).await;

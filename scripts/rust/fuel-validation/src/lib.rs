...
impl Trip {
    ...
    pub fn range(&self) -> DateRange {
        DateRange::from_dates(self.entries[0].date, self.entries.last().unwrap().date).unwrap()
    }
}
...
pub async fn run_sille_marie() -> Result<()> {
    let vessel_id = 2023124435;

    let mut trips = decode_sille_marie()?;
...
pub async fn run_ramoen() -> Result<()> {
    let vessel_id = 2016073913;

    let mut trips = decode_ramoen()?;
...
pub async fn run_heroyfjord_eros(bytes: &[u8], vessel_id: i64, name: &str) -> Result<()> {
...
pub async fn run_nergard() -> Result<()> {
    let trips = decode_nergard()?;
...
pub fn decode_sille_marie() -> Result<Vec<Trip>> {
    let bytes = [
...
pub fn decode_ramoen() -> Result<Vec<Trip>> {
